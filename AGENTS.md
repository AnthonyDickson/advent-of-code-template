...
Toolchains are pinned to explicit versions in `nix/devshells.nix`, and the shared tools (`just`, `fd`, `dprint`, `uv`,
...
- OCaml and Zig get their packages from the nix dev shell rather than a package manager, so their versions move with
  ...
  --save`.
- The lottery tool is not a language template: it pins `textual` in `tools/lottery/pyproject.toml` and commits its
  `uv.lock`, updated with `just lottery-update` (`uv lock --upgrade`).
...
- The lottery skips build output and `input.txt` when it copies a template, so a bootstrapped folder never inherits a
  stale puzzle input or a compiled binary. It writes `<year>-day-<dd>/` folders in the repository root, which are
  committed like any other solution.
