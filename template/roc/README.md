...
`nix`, run `nix develop .#roc` from anywhere inside the repository to enter a dev shell with all of the dependencies
installed. The dev shell provides a pinned nightly build of Roc's new compiler, which is how Roc currently ships; the
version nixpkgs packages is the older alpha4 line.
